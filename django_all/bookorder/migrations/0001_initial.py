# Generated by Django 3.0.5 on 2020-05-15 07:26

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
        ('booktype', '0003_bookstore'),
    ]

    operations = [
        migrations.CreateModel(
            name='BookOrder',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('date_order', models.DateField(help_text='YY-MM-DD', max_length=8)),
                ('amount', models.CharField(max_length=4)),
                ('bookstore', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='booktype.BookStore')),
                ('employee', models.ForeignKey(default=None, on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
        ),
    ]
